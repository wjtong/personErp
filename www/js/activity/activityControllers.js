 ...
 * */
  ...
  .controller('ActivityList', function ($scope, $location, $rootScope, $stateParams, ActivityServer) {
    ...
    if (type == 'finish') {
      ...
      ActivityServer.myActivity(roleTypeId, function (data) {
  ...
  .controller('ActivityDetails', function ($stateParams, $state, ThemeImage, ionicDatePicker, $scope, ActivityServer,
    ...
    ActivityServer.getActivityDetails($scope.workEffortId, function (data) {
      ...
      if ($scope.createPersonInfoList.activityAdminPartyId == $scope.partyId) {
        $scope.activityDetailsFootButton = '邀请';
        $scope.editButton = '编辑';
      } else {
        $scope.activityDetailsFootButton = '报名';
        $scope.editButton = '';
        if ($scope.groupMemberList) {
          for (var i = 1; i < $scope.groupMemberList.length; i++) {
            if ($scope.groupMemberList[i].partyId == $scope.partyId) {
              $scope.activityDetailsFootButton = '已报名';
              var sign = $('#sign').attr('disabled', true)
            }
          }
        }
      }
        ...
        buttonClicked: function (index) {
          switch (index) {
            ...
            case 1:
              ...
              break;
            case 2:
              $state.go('tab.activityPreview', {workEffortId: $scope.workEffortId});
              break;
            default:
              alert('请选择')
          }
    ...
    //提交编辑
    $scope.edit = true;
    $scope.deletePerson = false;
    $scope.editEvent = function (editButton) {
      if (editButton == '编辑') {
        $scope.edit = false;
        $scope.deletePerson = true;
        $scope.editButton = '完成';
        $('input').css({
          'color': 'red'
        });
        $('textarea').css('color', 'red');
      } else if (editButton == '完成') {
        ...
        confirmPopup.then(function (res) {
          if (res) {
            ...
            if ($scope.ActivityData.startDate == '开始时间待定') {
              $scope.ActivityData.startDate = ''
            }
            if ($scope.ActivityData.endDate == '结束时间待定') {
              $scope.ActivityData.endDate = ''
            }
            ActivityServer.updateActivity(
              ...
              $scope.workEffortId,
              null,
              $scope.activityList.serviceLoaderName,
              function (data) {
                console.log(data);
              }
            );
    ...
    //照片墙幻灯片
    $scope.shouBigImage = function (index, name) {
      if (name == undefined) {
        $scope.upLoadImage();
      } else {
   ...
   * Date 2017-6-26
   * */
  .controller('activityParticipant', function ($scope, ActivityServer, $stateParams) {
    //准备参数
    $scope.workEffortId = $stateParams.workEffortId;

    $scope.$on('$ionicView.enter', function () {
      //查询活动已参与人员
      ActivityServer.queryActivityMembers($scope.workEffortId, function (data) {
        $scope.activityParticipantList = data.activityMembersList
      });

      //查询活动待审批人员列表
      ActivityServer.queryActivityWaitApprovals($scope.workEffortId, function (data) {
        $scope.activityWaitApprovalstList = data.activityWaitApprovalList
      });
    });
    ...
    //活动批准
    $scope.approvalPartyJoin = function (id) {
      ActivityServer.approvalPartyJoin($scope.workEffortId, id, function (data) {
        console.log(data);
        if(data.resultMsg=='成功'){
          //查询活动已参与人员
          ActivityServer.queryActivityMembers($scope.workEffortId, function (data) {
  ...
  .controller('SharingActivity', function ($scope, $stateParams, Contact, $state, Login, $ionicPopup, $cordovaSms,
                                           $ionicPopover, ActivityServer, $cordovaClipboard) {
  ...
  .controller('ActivityMatter', function ($scope, $state, $stateParams, $ionicPopup, ActivityServer, $timeout) {
    ...
    //新建活动事项
    $scope.newActivityMatter = function () {
      $scope.itemMatter = {};
      var itemPopup = $ionicPopup.show({
        template: '<input type="text" placeholder="名称" ng-model="itemMatter.surveyName" style="background-color: white"><br>' +
        '<textarea rows="3" placeholder="内容" ng-model="itemMatter.questions"></textarea><br>' +
    ...
    $scope.createLabel = function () {
      if ($scope.itemMatter.surveyName == null || $scope.itemMatter.name == '') {
        alert('请填写活动项内容');
        $scope.addLab.close();
      } else {
        console.log($scope.itemMatter.surveyName + $scope.itemMatter.questions);
        ActivityServer.createActivityJob($scope.workEffortId, $scope.itemMatter.surveyName, $scope.itemMatter.questions, function (data) {
    ...
    //事项参与方式
    $scope.joinMatter = function (surveyId, surveyQuestionId) {
      var joinMatter = {};
      var myPopupMatter = $ionicPopup.show({
        template: '<textarea rows="3" ng-model="joinMatter.content"></textarea>',
        title: '参与方式',
        subTitle: '请输入您想要参与的内容',
        scope: $scope,
        buttons: [
          {text: '取消'},
          {
            ...
            type: 'button-positive',
            onTap: function (e) {
              if (!$scope.joinMatter.content) {
                alert('请输入内容');
                e.preventDefault();
              } else {
                ActivityServer.pushMyJob(surveyId, surveyQuestionId, $scope.joinMatter.content, function (data) {
                  if (data.resultMsg == '成功') {
                    var myPopup = $ionicPopup.show({
                      template: '',
                      title: '参与成功',
  ...
  .controller('ActivityDiscuss', function ($scope, $state, $location, $rootScope, $stateParams, ActivityServer, $ionicPopup,
    ...
    //查询讨论组相关信息
    ActivityServer.queryActivityCommunication($scope.communicationEventId, function (data) {
    ...
    $scope.openResponse = function (contentId) {
      ...
      $scope.Response.show();
      ActivityServer.queryActivityCommunicationResponce(contentId, function (data) {
    ...
    $scope.sentResponse = function () {
      ActivityServer.createActivityCommunication($scope.communicationEventId, $scope.contentId, $scope.Response.message, function (data) {
        if (data.resultMsg == '评论成功') {
          var myPopup = $ionicPopup.show({
            template: '',
            title: '发表成功',
   ...
   * Date 2017-3-3
   * */
  .controller('activityPreview', function ($scope, $stateParams, ActivityServer, $ionicPopup, $cordovaClipboard) {
    ...
    ActivityServer.printActivityInfo($scope.workEffortId, function (data) {
      $scope.activityInfo = data.activityInfo;
    });

    //复制链接
    $scope.Clipboard = function (activityInfo) {
      $ionicPopup.alert({
        title: '复制成功',
