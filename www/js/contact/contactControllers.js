  ...
  .controller('MainCtrl', function ($scope,$state,$ionicPopup,$ionicSlideBoxDelegate, $ionicScrollDelegate, filterFilter, $location, $anchorScroll, Contact,ActivityServer) {
    ...
    if ($scope.allPersonList != null) {
      for (var j = 0; j < $scope.allPersonList.length; j++) {
        array.push({
          'last_name': ConvertPinyin($scope.allPersonList[j].firstName).toUpperCase(),
    ...
    array
      ...
      .forEach(function (person) {
        ...
        var personCharCode = ConvertPinyin(person.last_name).toUpperCase().charCodeAt(0);
    ...
    $scope.getContacts = function () {
      ...
      return contacts.filter(function (item) {
        ...
        if (!item.isLetter && itemDoesMatch) {
          ...
          var letter = ConvertPinyin(pinyin).substring(0, 1);
